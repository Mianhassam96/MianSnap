...
export default function BottomPanel() {
  ...
  const s = {
    ...
    title: { fontSize: 11, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: 1 },
    controls: { display: 'flex', alignItems: 'center', gap: 6, flex: 1, justifyContent: 'center' },
    ctrlBtn: {
    ...
    time: { fontSize: 11, color: theme.textMuted, minWidth: 80, textAlign: 'center' },
    scrubber: { flex: 1, accentColor: theme.accent },
    body: { display: 'flex', gap: 0, height: 110 },
    videoWrap: {
      width: 180, flexShrink: 0, background: '#000', position: 'relative',
    ...
    framesWrap: { flex: 1, display: 'flex', gap: 4, padding: '6px 8px', overflowX: 'auto', alignItems: 'center' },
    thumb: (active) => ({
      height: 90, aspectRatio: '16/9', objectFit: 'cover', borderRadius: 4, cursor: 'pointer', flexShrink: 0,
      border: `2px solid ${active ? theme.accent : 'transparent'}`, transition: 'border-color 0.15s',
    }),
    extractBtn: {
      padding: '5px 10px', borderRadius: 5, border: 'none',
      background: theme.accent, color: '#fff', fontSize: 11, cursor: 'pointer', flexShrink: 0,
    },
    captureBtn: {
      padding: '5px 10px', borderRadius: 5, border: `1px solid ${theme.border}`,
      background: theme.bgTertiary, color: theme.text, fontSize: 11, cursor: 'pointer', flexShrink: 0,
    },
  }
    ...
    <div style={s.panel}>
      ...
      <div style={s.header}>
        ...
        <div style={s.controls}>
          ...
          <button style={s.playBtn} onClick={togglePlay}>{playing ? '⏸' : '▶'}</button>
          <button style={s.ctrlBtn} onClick={() => handleStep(1)}>› ⏭</button>
          <input type="range" style={s.scrubber} min={0} max={duration || 100} step={0.01}
      ...
      <div style={s.body}>
        ...
        <div style={s.framesWrap}>
          {frames.length === 0 && (
            <span style={{ fontSize: 11, color: theme.textMuted }}>
              {videoUrl ? 'Click "Extract Frames" to generate thumbnails' : 'Upload a video to get started'}
            </span>
          )}
          {frames.map((f, i) => (
            <img key={i} src={f.dataUrl} alt={`${f.time}s`}
              style={s.thumb(selectedFrame?.time === f.time)}
              onClick={() => handleFrameClick(f)}
            />
          ))}
        </div>
