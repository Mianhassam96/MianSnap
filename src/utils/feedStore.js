...
const SEED_CLAIMS = [
  ...
  { id:'s5',  text:"They don't want you to know! Deep state is controlling all major news networks!!!", score:8,  checks:4201, category:'Politics' },
  ...
  { id:'s10', text:'AP News: Ultra-processed food linked to 32% higher cardiovascular risk in new study', score:79, checks:2341, category:'Health' },
  { id:'s11', text:'URGENT: Share now — government will shut down internet in 48 hours!!!', score:5,  checks:7234, category:'Politics' },
  { id:'s12', text:'Stanford: Remote work increases productivity by 13% in 16,000-person study', score:77, checks:1456, category:'Business' },
]
...
export function initFeed() {
  ...
  const feed = SEED_CLAIMS.map(c => ({
    ...c,
    checks: c.checks + Math.floor(Math.random() * 200),
    addedAt: Date.now() - Math.floor(Math.random() * 86400000 * 3),
    isUserAdded: false,
  }))
  saveFeed(feed)
  return feed
...
export function addToFeed(text, score) {
  ...
  const entry = {
    id: 'u' + Date.now(),
    text: claim + (claim.length >= 120 ? '…' : ''),
    score,
    checks: 1 + Math.floor(Math.random() * 12),
    category: 'User Submitted',
    addedAt: Date.now(),
    isUserAdded: true,
  }
  const updated = [entry, ...feed.filter(f => f.isUserAdded).slice(0, 7), ...feed.filter(f => !f.isUserAdded)].slice(0, 20)
  saveFeed(updated)
  return updated
}
...
export function incrementSessionStats() {
  ...
  s.totalChecked = (s.totalChecked || 0) + 1
